...
/**
 * This Object contains the data for one relative ranking by encoding it into
 * two arrays: The compared objects and the compare operators.
 * 
 ...
 * </ul>
 * In the case of two operators following each other, e.g. " &gt; {" these
 * corresponding values of "3" and "5" are concatenated into a "35".
 * 
...
public class Ranking {
	...
	/**
	 * Creates a {@link Ranking} object which encodes ranking information. It
	 * consists of an array of compared objects and the array of used compare
	 * operators.
	 * 
	 * @param objectList
	 *            an array of object IDs which are ranked
	 * @param compareOperators
	 *            an array of the used compared operators
	 */
	 ...
	 *             if the character is not a valid character for {@link Ranking}
	 */
	...
	/**
	 * Returns the ordering for ranking. The ranking objects should start from 1 in
	...
	/**
	 * Returns the ordering for the given ranking. The ranking objects should start
	 * from 1 in increasing order. This method is not applicable for partial
	 * ranking.
	 *
	 * @param rankingObjectList
	 *            the ranking object list
	 * @return the ordering for ranking
	 */
	public static int[] getOrderingFromRanking(int[] rankingObjectList) {
		int[] orderingForRanking = new int[rankingObjectList.length];

		// in case there is a ranking which is starting with 1 or 0
		boolean startFromZero = true;
		if (Arrays.stream(rankingObjectList).min().getAsInt() == 1) {
			startFromZero = false;
		}
		...
		for (int i = 0; i < orderingForRanking.length; i++) {
			if (startFromZero) {
				...
				orderingForRanking[rankingObjectList[i] - 1] = i;
			}
